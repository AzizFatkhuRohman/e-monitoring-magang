...
class AuthentifikasiController extends Controller
{
    ...
    public function lupa_password(Request $request) {
        if ($request->has('submit')) {
            $this->validate($request, [
                'nomor_induk'           => 'required|exists:users,nomor_induk'
            ], [
                'nomor_induk.required'  => 'NIM Wajib diisi',
                'nomor_induk.exists'    => 'NIM tidak terdaftar'
            ]);

            $request->session()->put('token', Str::random(64));
            if (ResetPasswordModel::tokenModel($request->nim, $request->session()->get('token'))) {
